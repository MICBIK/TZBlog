...
TZBlog 已完成设计基线、第一阶段工程骨架初始化，以及 Astro 前台到 Payload CMS REST API 的主内容数据链路接入。当前仓库已经具备 `Astro + Payload CMS + PostgreSQL` 的最小可运行结构，并完成了主要内容页面的数据源切换与静态收口。
...
- 已完成：Payload 内容 collections（`posts / projects / docs / notes`）与 Astro 前台主内容数据链路接入
- 已完成：前台去除示例内容 fallback 主链路，改为 API 不可用时 empty state
- 已验证：静态代码、OpenSpec 与主方案文档已完成一轮对账
- 进行中：运行时验收、搜索真实索引、统计接入、Payload globals 与更完整后台能力
