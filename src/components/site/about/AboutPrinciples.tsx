      ...
      <div className="grid gap-4 md:grid-cols-3">
        {items.map((item) => (
          <article key={item.label} className="launch-panel space-y-3 p-5">
            <h3 className="font-mono text-[var(--text-label)] tracking-[var(--tracking-label)] uppercase text-muted-fg">
              {item.label}
            </h3>
            <p className="font-serif text-[var(--text-base)] leading-[var(--leading-body)] text-fg">
              {item.detail}
            </p>
          </article>
        ))}
