...
| CMS（自研） | 登录、专栏 CRUD、文章 CRUD、Tiptap WYSIWYG（存 Markdown）、实时预览、媒体上传 |
...
- **V2**：主题系统（CSS 变量 + 后台 GUI 编辑色板 + 一键切换）；详细 Analytics 仪表盘（来源/设备/国家）；评论邮件通知；可能接 Giscus 作为评论备选。
- **V3**：多语言（zh / en），schema 已预留 `*Translation` 子表，加 locale 即可。
...
- Tiptap-markdown 对复杂语法（表格/脚注/数学）支持参差 → MVP 不上数学/脚注
