...
## 三轮验收（D1/D2 修复后，2026-06-14 晚）

### ✅ D1 已修复
迁移 000005_add_is_premium_column 已添加。全新建库后 articles 表含 is_premium 列。文章创建（含 isPremium/categoryId/tags）完全通过。

### ⚠️ D2 部分修复 — 新发现 D2b（userID context key 不匹配）
like_handler 已改用多态查询（TargetTypeArticle），但点赞仍返回 500。

**根因**：认证中间件与 like/follow/payment handler 的 context key 命名不一致：

| 位置 | key | 状态 |
|---|---|---|
| middleware/auth.go:48 | `c.Set("user_id", ...)` | ✅ 正确 |
| article_handler.go | `c.GetInt64("user_id")` | ✅ 正确 |
| auth_handler.go | `c.GetInt64("user_id")` | ✅ 正确 |
| comment_handler.go | `c.GetInt64("user_id")` | ✅ 正确 |
| **like_handler.go:37** | `c.GetInt64("userID")` | ❌ **应为 "user_id"** |
| **follow_handler.go** | `c.GetInt64("userID")` | ❌ **应为 "user_id"** |
| **payment_handler.go** | `c.GetInt64("userID")` | ❌ **应为 "user_id"** |

**修复**：把 like_handler.go / follow_handler.go / payment_handler.go 里所有 `c.GetInt64("userID")` 改为 `c.GetInt64("user_id")`。

**错误**：`INSERT INTO "likes" (user_id,...) VALUES (0,...)` — user_id 为 0，违反 fk_likes_user 外键。

### 联调结果（8/9 通过）
前端连真实后端（NEXT_PUBLIC_USE_MOCK=false）验证：
- ✅ 首页渲染后端文章数据
- ✅ 文章列表 + 详情（Markdown 渲染）
- ✅ 登录/注册页表单
- ✅ 文章 CRUD（创建/更新/删除）
